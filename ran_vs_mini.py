...
# 対戦の進行を管理する関数（ランダムAI vs Minimax AI）
def play_game_random_vs_minimax():
    board = init_board()
    is_black_turn = True  # 黒がランダムAI、白がMinimax AIとします
    ...
    while True:
        ...
        if is_black_turn:
            ...
            _, move = minimax(board, 3, is_black_turn, float('-inf'), float('inf'))
...
def simulate_games(num_games):
    ...
    for _ in range(num_games):
        final_board = play_game_random_vs_minimax()
    ...
    print(f"ランダムAIの勝利数: {random_wins}")
    print(f"Minimax AIの勝利数: {minimax_wins}")
    print(f"引き分け数: {draws}")
